...
public class Shooter extends SubsystemBase {
  ...
  /// *** ATTRIBUTES ***
  //////////////////////////////////
  WPI_TalonFX m_flywheelMotorMaster;  /**< Master motor used to drive the flywheels */
  WPI_TalonFX m_flywheelMotorSlave;   /**< Slave motor used to drive the flywheels */
  WPI_TalonFX m_indexMotor;           /**< Motor used to drive the ball indexer */
  ...
  public boolean getBallLimitSensor() { return !m_ballLimitSensor.get(); }

  /** Returns true when a ball is present in the upper end of the shooter */
  public boolean upperBallIsPresent() { return !m_ballSensorMiddle.get(); }

  /** Returns true when a ball is present in the lower end of the shooter */
  public boolean lowerBallIsPresent() { return !m_ballSensorLower.get(); }
  ...
  public enum BallIndexerMode {
    Stopped (0.0),    /**< Ball indexer is stopped */
    FeedBall (0.7),   /**< Run forward to feed balls into the shooter */
    ShootBall (1.0),  /**< Run forward to move a ball into the flywheel */
    Reverse (-0.4);   /**< Run indexer in reverse */

    private final double motorSpeed;
    private BallIndexerMode(double speed) {motorSpeed = speed;}
   ...
   * @param shouldRun  true to run the flywheel, false to stop it
  */
  public void runFlywheel(boolean shouldRun) {
    double speed = 0.0;
    
    if (shouldRun) {
      double invert = (kInvertFlywheelMotor ? -1.0 : 1.0);
      speed = Constants.ShooterConstants.kFlywheelSpeed * invert;
    }
    m_flywheelMotorMaster.set(TalonFXControlMode.PercentOutput, speed);
  }
